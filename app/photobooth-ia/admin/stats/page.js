...
export default function StatsPage() {
  ...
  const fetchStats = useCallback(async () => {
    ...
    try {
      ...
      console.log(`Found ${projectsData?.length || 0} projects for this user`);
      ...
      let photosByMonth = [];
      try {
        // Fetch only sessions related to user's projects
      ...
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
          ...
          {session?.company_name 
            ? `Statistiques de ${session.company_name}`
            : "Vos statistiques"}
        </h1>
        <button
          onClick={fetchStats}
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          <div className="flex-1">
            {stats.topProjects.length === 0 ? (
              <div className="text-gray-500">Aucun projet trouvé ou aucune photo générée.</div>
            ) : (
