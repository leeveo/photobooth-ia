...
export default function StyleSelection({ params }) {
  ...
  useEffect(() => {
    fetchStyles();
  }, [fetchStyles]); // Add fetchStyles to dependencies
  ...
  }, [gender, slug]); // Add gender and slug to dependencies
  ...
  const handleSkip = useCallback(() => {
    if (gender && project?.id) {
      // Rediriger vers la page caméra avec les paramètres par défaut
      router.push(`/photobooth-premium/${slug}/cam?gender=${gender}&style=default`);
    } else {
      setError("Impossible de passer, veuillez sélectionner au moins un style");
    }
  }, [gender, project.id]); // Add gender and project.id to dependencies
