...
const useWebcam = ({ videoRef, setCameraError, setCameraLoaded }) => {
  useEffect(() => {
    ...
    const tryInitCamera = async (constraints) => {
      try {
        ...
        if (!isMounted) {
          ...
          // DO NOT STOP TRACKS HERE IF WE WANT PERSISTENCE, BUT HERE IT'S A NEW STREAM
          // If we are unmounting, we probably don't want to keep this new stream if we haven't saved it yet
          // But let's be safe and stop it if we are unmounting during init
          stream.getTracks().forEach(track => track.stop());
          return null;
        }
    ...
    const initializeCamera = async () => {
      ...
      // 1. Check for existing persistent stream first
      if (window.persistentCameraStream && window.persistentCameraStream.active) {
        const tracks = window.persistentCameraStream.getVideoTracks();
        if (tracks.length > 0 && tracks[0].readyState === 'live') {
          console.log("♻️ Using existing persistent camera stream");
          streamCam = window.persistentCameraStream;
      ...
      window.persistentCameraStream = stream; // Store in persistent global variable
    ...
    const attachStreamToVideo = async (retryCount = 0) => {
      // Use streamCam which should be set by now (either from new init or persistent)
      const stream = streamCam || window.persistentCameraStream;
    ...
    return () => {
      ...
      // MODIFICATION: Ne PAS arrêter les tracks pour garder la permission active sur iPad
      // On garde le stream actif dans window.persistentCameraStream
      /*
      if (streamCam) {
        try {
      ...
      */
     console.log("⚠️ Camera cleanup skipped to maintain permission on iPad");
    };
...
export default function CameraCapture({ params }) {
  ...
  const retryCamera = useCallback(() => {
    ...
    const streamToStop = streamCam || window.persistentCameraStream;
    if (streamToStop) {
      try {
        streamToStop.getTracks().forEach(track => track.stop());
        streamCam = null;
        window.persistentCameraStream = null;
        window.localStream = null;
      } catch (e) {
  ...
  const switchCamera = async () => {
    ...
    try {
      ...
      if (newStream) {
        ...
        window.persistentCameraStream = newStream; // Update persistent stream
